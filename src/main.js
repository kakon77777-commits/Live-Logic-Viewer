...
let store=null,controller=null,previous=null
...
remoteForm.addEventListener('submit',async event=>{event.preventDefault();if(remoteRun.disabled)return;remoteRun.disabled=true;setStatus('Submitting isolated execution job…');const token=remoteToken.value;remoteToken.value='';try{const envelope=await submitRemoteExecution({source:remoteSource.value,accessToken:token});renderExecutionResult(resultRoot,envelope);setStatus(`Managed execution ${envelope.status}. Result was not promoted to evidence.`)}catch(error){setStatus(error.message,true)}finally{remoteRun.disabled=false}})
clearExecutionResult(resultRoot);loadBuiltInDemo()
