...
import { readBoundedUtf8Blob, readBoundedUtf8Message } from '../shared/bounded-utf8.js'
...
async function loadBuiltInDemo(){try{const response=await fetch('/examples/demo-events.json',{credentials:'same-origin'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const text=await readBoundedUtf8Message(response,MAX_PACKAGE_BYTES,'built-in event demo');loadPackage(parseAndValidatePackage(text),'Built-in demo')}catch(error){setStatus(`Demo load failed: ${error.message}`,true)}}
async function loadExecutionDemo(){try{const response=await fetch('/examples/demo-execution-result.json',{credentials:'same-origin'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const text=await readBoundedUtf8Message(response,MAX_EXECUTION_RESULT_BYTES,'built-in execution demo');const envelope=parseAndValidateExecutionResult(text);setExecutionEnvelope(envelope,'Safe execution-result demo loaded. Judgment state was not modified.',envelope.integrity?'present-unverified':'unverified')}catch(error){setStatus(`Execution demo load failed: ${error.message}`,true)}}
...
fileInput.addEventListener('change',async()=>{const file=fileInput.files?.[0];if(!file)return;try{const text=await readBoundedUtf8Blob(file,MAX_PACKAGE_BYTES,`event package ${file.name}`);loadPackage(parseAndValidatePackage(text),file.name)}catch(error){setStatus(`Import rejected: ${error.message}`,true)}finally{fileInput.value=''}})
resultFileInput.addEventListener('change',async()=>{const file=resultFileInput.files?.[0];if(!file)return;try{const text=await readBoundedUtf8Blob(file,MAX_EXECUTION_RESULT_BYTES,`execution result ${file.name}`);const envelope=parseAndValidateExecutionResult(text);const integrityStatus=await classifyImportedExecutionIntegrity({envelope,capabilities:executionCapabilities,refreshCapabilities});setExecutionEnvelope(envelope,`${file.name} inspected as execution data only. No evidence or judgment was changed.`,integrityStatus)}catch(error){setStatus(`Execution result rejected: ${error.message}`,true)}finally{resultFileInput.value=''}})
