...
const safe = () => ({
  ...
  limits:{max_source_bytes:65536,min_wall_ms:100,max_wall_ms:10000,min_output_bytes:1024,max_output_bytes:65536},
  execution_result_is_evidence:false,
  result_integrity:{required:false,algorithm:'ECDSA_P256_SHA256',key_id:null,public_jwk:null}
})
...
describe('control-plane capabilities handshake',()=>{
  it('accepts only the narrow v0.1 capability contract',()=>{const value=validateExecutionCapabilities(safe());expect(value.runners).toEqual(['python']);expect(value.network_policies).toEqual(['deny']);expect(safeExecutionLimits(value)).toEqual({max_source_bytes:65536,wall_ms:5000,output_bytes:65536})})
  it('accepts a required P-256 result-signing contract',()=>{const value=safe();value.result_integrity={required:true,algorithm:'ECDSA_P256_SHA256',key_id:'prod-2026-01',public_jwk:publicJwk};expect(validateExecutionCapabilities(value).result_integrity.required).toBe(true)})
  it('rejects required signing without a public key',()=>{const value=safe();value.result_integrity={required:true,algorithm:'ECDSA_P256_SHA256',key_id:'prod',public_jwk:null};expect(()=>validateExecutionCapabilities(value)).toThrow(/public_jwk|object/i)})
