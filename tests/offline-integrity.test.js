...
async function envelope(){return{schema_version:'0.1',job_id:'offline-job',request_id:request.request_id,received_at:'2026-08-17T06:00:00.000Z',completed_at:'2026-08-17T06:00:00.002Z',status:'completed',execution:{provider:'mock',runner:'python',exit_code:0,timed_out:false,wall_ms:2},result:{type:'text',stdout:'2\n',stderr:'',truncated:false},provenance:{source_sha256:await sha256Hex(request.source),request_sha256:await sha256Hex(canonicalExecutionRequest(request))}}}
...
describe('offline execution-result integrity classification',()=>{
  ...
  it('marks a signature unverified when no trust context exists',async()=>{const key=await keyFixture();const value=await envelope();value.integrity=await signExecutionResult(value,key.privateJwk,key.keyId);expect(value.integrity.payload_version).toBe('2');expect(await classifyImportedExecutionIntegrity({envelope:value,capabilities:null})).toBe('present-unverified')})
  ...
  it('rejects signed v2 time tampering',async()=>{const key=await keyFixture();const value=await envelope();value.integrity=await signExecutionResult(value,key.privateJwk,key.keyId);value.completed_at='2026-08-17T06:00:09.999Z';await expect(classifyImportedExecutionIntegrity({envelope:value,capabilities:caps([key],true)})).rejects.toThrow(/signature verification failed/i)})
})
