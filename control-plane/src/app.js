...
function corsHeaders(request, env) {
  ...
  const allowed = env?.VIEWER_ORIGIN
  if (!origin) return {}
  if (!allowed || origin !== allowed) return null
  return { 'access-control-allow-origin': allowed, 'access-control-allow-methods': 'POST, OPTIONS', 'access-control-allow-headers': 'authorization, content-type', 'vary': 'Origin' }
}
function authorized(request, env) {
  const token = env?.CONTROL_API_TOKEN
  if (typeof token !== 'string' || token.length < 16) return 'misconfigured'
